...
const uploadFilesSchema = z.object({
  ...
  zipContent: z
    ...
    .refine(
      value =>
        Buffer.from(value, "base64").length <= MAX_LEGACY_BASE64_ZIP_BYTES,
      {
...
function deriveProjectAnalysisStatus(
...
export function toTrpcError(error: unknown): TRPCError {
  ...
  if (error instanceof AppError) {
    const codeMap: Record<AppError["code"], TRPCError["code"]> = {
      ...
      UPLOAD_TEMP_CLEANUP_ACTIVE_JOB_LOOKUP_FAILED: "INTERNAL_SERVER_ERROR",
      UPLOAD_TEMP_CLEANUP_ACTIVE_JOB_PAYLOAD_INVALID: "INTERNAL_SERVER_ERROR",
      JOB_STALE_MAX_ATTEMPTS: "INTERNAL_SERVER_ERROR",
      PROJECT_JOB_TIMEOUT: "INTERNAL_SERVER_ERROR",
      PROJECT_JOB_WORKER_EXITED: "INTERNAL_SERVER_ERROR",
      ...
      ANALYSIS_PERSIST_FAILED: "INTERNAL_SERVER_ERROR",
      ANALYSIS_SUMMARY_FAILED: "INTERNAL_SERVER_ERROR",
      ANALYSIS_UNKNOWN_FAILED: "INTERNAL_SERVER_ERROR",
      REPORT_NOT_READY: "CONFLICT",
      UNSUPPORTED_SNAPSHOT_VERSION: "CONFLICT",
...
function toProjectSummary(
  project:
    | Awaited<ReturnType<typeof getOwnedProject>>
    | Record<string, unknown>,
  analysisStatus: AnalysisStatus,
  latestJob: ReturnType<typeof getLatestJobsByProjectIds> extends Promise<
    Map<number, infer T>
  >
    ? T | null
    : never
  ...
  return {
    ...
    sourceType: project.sourceType as ReturnType<
      typeof projectSourceTypeSchema.parse
    >,
    ...
    errorMessage:
      typeof project.errorMessage === "string" ? project.errorMessage : null,
    lastErrorCode:
      typeof project.lastErrorCode === "string" ? project.lastErrorCode : null,
    importWarningsJson: Array.isArray(project.importWarningsJson)
      ? project.importWarningsJson
      : [],
    sourceFingerprint:
      typeof project.sourceFingerprint === "string"
        ? project.sourceFingerprint
        : null,
    lastAnalyzedAt:
      project.lastAnalyzedAt instanceof Date
        ? project.lastAnalyzedAt
        : project.lastAnalyzedAt
          ? new Date(String(project.lastAnalyzedAt))
          : null,
    createdAt:
      project.createdAt instanceof Date
        ? project.createdAt
        : project.createdAt
          ? new Date(String(project.createdAt))
          : new Date(0),
    updatedAt:
...
export const appRouter = router({
  ...
  projects: router({
    list: protectedProcedure
      .output(projectsListOutputSchema)
      .query(async ({ ctx }) => {
        try {
          ...
          const analysisStatusByProjectId =
            await getProjectApiAnalysisStatusByProjectId(db, projectRows);
          const latestJobsByProjectId = await getLatestJobsByProjectIds(
          ...
          const visibleProjectRows = projectRows.filter(
            project =>
              ...
              latestJobsByProjectId.has(project.id)
          );

          return visibleProjectRows.map(project =>
            toProjectSummary(
              project,
              deriveProjectAnalysisStatus(
                project.status,
                analysisStatusByProjectId.get(project.id)
              ),
    ...
    getById: protectedProcedure
      ...
      .output(projectByIdOutputSchema)
      .query(async ({ ctx, input }) => {
        try {
          ...
          const analysisStatusByProjectId =
            await getProjectApiAnalysisStatusByProjectId(db, [project]);
          const latestJobsByProjectId = await getLatestJobsByProjectIds(
            [input],
            ctx.user.id
          );
          ...
          return toProjectSummary(
            project,
            analysisStatusByProjectId.get(input) ??
              deriveProjectAnalysisStatus(project.status),
            latestJobsByProjectId.get(input) ?? null
          );
        } catch (error) {
          raiseAsTrpc(error);
        }
    ...
    uploadFiles: protectedProcedure
      ...
      .output(projectJobCreateResultSchema)
      .mutation(async ({ ctx, input }) => {
        try {
          return await queueImportProjectZip(
            input.projectId,
            ctx.user.id,
            input.zipContent
          );
    ...
    cloneGit: protectedProcedure
      .input(cloneGitSchema)
      .output(projectJobCreateResultSchema)
      .mutation(async ({ ctx, input }) => {
        try {
          return await queueImportProjectGit(
            input.projectId,
            ctx.user.id,
            input.gitUrl
          );
  ...
  analysis: router({
    ...
    getSnapshot: protectedProcedure
      .input(projectIdSchema)
      .output(analysisSnapshotOutputSchema)
      .query(async ({ ctx, input }) => {
        try {
    ...
    listRuns: protectedProcedure
      .input(analysisRunListInputSchema)
      .output(analysisRunsPageOutputSchema)
      .query(async ({ ctx, input }) => {
        try {
          return await listAnalysisRunsForProject(
            input.projectId,
            ctx.user.id,
            input.page,
            input.pageSize
    ...
    getRun: protectedProcedure
      ...
      .output(analysisRunDetailOutputSchema)
      .query(async ({ ctx, input }) => {
        try {
          return await getAnalysisRunForProject(
            input.projectId,
            ctx.user.id,
            input.runId
          );
    ...
    setBaseline: protectedProcedure
      .input(analysisRunInputSchema)
      .output(projectDeleteOutputSchema)
      .mutation(async ({ ctx, input }) => {
        try {
          return await setAnalysisBaselineForProject(
            input.projectId,
            ctx.user.id,
            input.runId
          );
    ...
    clearBaseline: protectedProcedure
      .input(projectIdSchema)
      .output(projectDeleteOutputSchema)
      .mutation(async ({ ctx, input }) => {
        try {
    ...
    getDiff: protectedProcedure
      .input(analysisDiffInputSchema)
      .output(analysisDiffOutputSchema)
      .query(async ({ ctx, input }) => {
        try {
          return await getAnalysisDiffForProject(
            input.projectId,
            ctx.user.id,
            input.baseRunId,
            input.compareRunId
    ...
    getFlowTracesPage: protectedProcedure
      .input(flowTracesPageInputSchema)
      .output(flowTracesPageOutputSchema)
      .query(async ({ ctx, input }) => {
        try {
          return await getFlowTracesPageForProject(
            input.projectId,
            ctx.user.id,
            input
          );
    ...
    getFlowTrace: protectedProcedure
      .input(flowTraceInputSchema)
      .output(flowTraceOutputSchema)
      .query(async ({ ctx, input }) => {
        try {
          return await getFlowTraceForProject(
            input.projectId,
            ctx.user.id,
            input.stableKey,
            input.runId
    ...
    getFlowTraceSummary: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        try {
          return await getFlowTraceSummaryForProject(
            input.projectId,
            ctx.user.id,
            input.runId
          );
    ...
    getSymbolsPage: protectedProcedure
      .input(symbolsPageInputSchema)
      .output(symbolsPageOutputSchema)
      .query(async ({ ctx, input }) => {
        try {
    ...
    getFieldsPage: protectedProcedure
      .input(fieldsPageInputSchema)
      .output(fieldsPageOutputSchema)
      .query(async ({ ctx, input }) => {
        try {
    ...
    getRisksPage: protectedProcedure
      .input(risksPageInputSchema)
      .output(risksPageOutputSchema)
      .query(async ({ ctx, input }) => {
        try {
    ...
    getRulesPage: protectedProcedure
      .input(rulesPageInputSchema)
      .output(rulesPageOutputSchema)
      .query(async ({ ctx, input }) => {
        try {
    ...
    getDependenciesPage: protectedProcedure
      .input(dependenciesPageInputSchema)
      .output(dependenciesPageOutputSchema)
      .query(async ({ ctx, input }) => {
        try {
    ...
    getFieldDependenciesPage: protectedProcedure
      .input(fieldDependenciesPageInputSchema)
      .output(fieldDependenciesPageOutputSchema)
      .query(async ({ ctx, input }) => {
        try {
    ...
    getImpact: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        try {
          return await runImpactAnalysis(
            input.projectId,
            ctx.user.id,
            input.target,
            input.type

...
function serializeSnapshot(snapshot: AnalysisRunSnapshotV1) {
  ...
  if (bytes > MAX_SNAPSHOT_BYTES) {
    throw new AppError(
      ...
      `Analysis snapshot is too large to persist (${bytes} bytes; limit ${MAX_SNAPSHOT_BYTES}).`
    );
  }
  return text;
}
...
): status is (typeof USABLE_STATUSES)[number] {
  return USABLE_STATUSES.includes(status as (typeof USABLE_STATUSES)[number]);
}

function getProjectContextOrLegacyDefault(
  snapshot: AnalysisRunSnapshotV1 | null | undefined
) {
...
function normalizeMetrics(result: ProjectAnalysisResult) {
  return {
    ...
    eligibleFileCount:
      result.metrics?.eligibleFileCount ?? result.metrics?.fileCount ?? 0,
    analyzedFileCount:
      result.metrics?.analyzedFileCount ?? result.metrics?.fileCount ?? 0,
    ...
    dependencyCount:
      result.metrics?.dependencyCount ?? result.dependencies?.length ?? 0,
    fieldCount: result.metrics?.fieldCount ?? result.schemaFields?.length ?? 0,
    fieldDependencyCount:
      result.metrics?.fieldDependencyCount ??
      result.fieldReferences?.length ??
      0,
...
function normalizeLegacyMetrics(
  metrics: ProjectAnalysisResult["metrics"] | null | undefined,
  counts: {
    files: number;
    symbols: number;
    dependencies: number;
  ...
  return {
    ...
    fieldDependencyCount:
      metrics?.fieldDependencyCount ?? counts.fieldDependencies,
...
export function parseAnalysisSnapshot(snapshotJson: string | null | undefined) {
  if (!snapshotJson) {
    return {
      snapshot: null,
      warning: "This legacy run does not have a persisted snapshot yet.",
    };
  }
  let raw: unknown;
  try {
  ...
  if (schemaVersion !== 1) {
    return {
      snapshot: null,
      warning: `Snapshot schema version ${String(schemaVersion)} is not supported by this Legacy Lens build.`,
    };
  }
  ...
  if (!parsed.success) {
    return {
      snapshot: null,
      warning: "Persisted snapshot failed strict validation.",
    };
  }
  return { snapshot: parsed.data, warning: null };
}

export function parseAnalysisSnapshotStrict(
  snapshotJson: string | null | undefined
) {
  if (!snapshotJson) {
    throw new AppError(
      "REPORT_NOT_READY",
      "Historical report snapshot is missing for the selected run."
    );
  }
  ...
  try {
    ...
    throw new AppError(
      "REPORT_NOT_READY",
      "Historical report snapshot JSON is invalid for the selected run."
    );
  }
  ...
  if (schemaVersion !== 1) {
    throw new AppError(
      ...
      `Snapshot schema version ${String(schemaVersion)} is not supported by this Legacy Lens build.`
    );
  }
  ...
  if (!parsed.success) {
    throw new AppError(
      "REPORT_NOT_READY",
      "Historical report snapshot failed strict validation for the selected run."
    );
  }
...
export function buildAnalysisRunSnapshot(
  ...
  const snapshot: AnalysisRunSnapshotV1 = {
    ...
    fields: sortByJson(
      (result.schemaFields ?? []).map(field => ({
        ...field,
        file: normalizePath(field.file),
        line: field.line ?? 0,
      }))
    ),
    fieldDependencies: sortByJson(
      (result.fieldReferences ?? []).map(reference => ({
        ...reference,
    ...
    risks: sortByJson(
      (result.risks ?? []).map(risk => ({
        ...risk,
        sourceFile: normalizePath(risk.sourceFile),
      }))
    ),
    rules: sortByJson(
    ...
    delphiEventMap: sortByJson(
      (result.delphiEventMap ?? []).map(entry => ({
        ...
        resolvedFile: entry.resolvedFile
          ? normalizePath(entry.resolvedFile)
          : null,
      }))
    ),
    delphiDataBindings: sortByJson(
      (result.delphiDataBindings ?? []).map(binding => ({
        ...binding,
        sourceFile: normalizePath(binding.sourceFile),
      }))
    ),
    sqlStatements: sortByJson(result.sqlStatements ?? []),
    buildDoctor: delphiBuildDoctorResultSchema.parse(
      result.buildDoctor ?? {
    ...
    flowTraceSummary: result.flowTraceSummary ?? {
      candidateTraceCount: result.flowTraces?.length ?? 0,
      persistedTraceCount: result.flowTraces?.length ?? 0,
  ...
  return {
    snapshot,
    sourceFingerprint: calculateSourceFingerprint(projectFiles),
    snapshotJson,
  };
}
...
export async function allocateNextRunNumber(db: DbHandle, projectId: number) {
  const rows = await db
    .select({ runNumber: analysisResults.runNumber })
    .from(analysisResults)
    .where(eq(analysisResults.projectId, projectId));
  return (
    rows.reduce((max, row) => Math.max(max, Number(row.runNumber ?? 0)), 0) + 1
  );
}
...
async function getBaselineId(db: DbHandle, projectId: number) {
  const [baseline] = await db
    ...
    .where(eq(analysisBaselines.projectId, projectId))
    .limit(1);
  return baseline?.analysisResultId ?? null;
}

export async function getLatestUsableAnalysisRun(
  db: DbHandle,
  projectId: number
) {
  const [report] = await db
    ...
    .orderBy(
      desc(analysisResults.runNumber),
...
export async function ensureProjectSourceFingerprint(
  db: DbHandle,
  projectId: number
) {
  ...
  await db
    .update(projects)
    .set({ sourceFingerprint: fingerprint, updatedAt: new Date() })
    .where(eq(projects.id, projectId));
  return fingerprint;
}

export async function getLatestUsableCurrentSourceRun(
  db: DbHandle,
  projectId: number
) {
  ...
  const [report] = await db
    ...
    .where(
      and(
        eq(analysisResults.projectId, projectId),
        eq(analysisResults.sourceFingerprint, sourceFingerprint),
        inArray(analysisResults.status, [...USABLE_STATUSES])
      )
    )
    .orderBy(
...
export async function getProjectApiAnalysisStatusByProjectId(
  ...
  const projectsNeedingFingerprint = projectRows.filter(
    project => !project.sourceFingerprint
  );
  if (projectsNeedingFingerprint.length > 0) {
    const fileRows = await db
      ...
      .where(
        inArray(
          files.projectId,
          projectsNeedingFingerprint.map(project => project.id)
        )
      );
  ...
  const currentFingerprints = [
    ...new Set(currentFingerprintByProjectId.values()),
  ];
  ...
  const sortedRuns = [...usableCurrentRuns].sort((left, right) => {
    const runNumberDelta =
      Number(right.runNumber ?? 0) - Number(left.runNumber ?? 0);
  ...
  for (const run of sortedRuns) {
    ...
    if (
      ...
      statusByProjectId.has(run.projectId)
    ) {
      continue;
    }
...
async function getUsableRunById(
  db: DbHandle,
  projectId: number,
  runId: number
) {
  ...
  if (!run) {
    throw new AppError(
      "PROJECT_NOT_FOUND",
      "Analysis run not found for this project."
    );
  }
  if (!isUsableStatus(run.status)) {
    throw new AppError(
      "REPORT_NOT_READY",
      "Historical analysis artifacts are only available for usable immutable runs."
    );
  }
  return run;
}
...
function toSummary(
  report: typeof analysisResults.$inferSelect,
  baselineId: number | null,
  latestUsableId: number | null
): AnalysisRunSummary {
  ...
  return {
    ...
    createdAt:
      report.createdAt instanceof Date
        ? report.createdAt
        : new Date(String(report.createdAt)),
    completedAt: report.completedAt
      ? report.completedAt instanceof Date
        ? report.completedAt
        : new Date(String(report.completedAt))
      : null,
...
export async function materializeLegacySnapshotIfMissing(
  db: DbHandle,
  projectId: number,
  runId?: number
) {
  ...
  if (missingSnapshots.length > 1) {
    const currentSourceMissing = missingSnapshots.filter(
      row =>
        row.sourceFingerprint &&
        row.sourceFingerprint === project.sourceFingerprint
    );
    if (!runId && currentSourceMissing.length === 1) {
      legacy = currentSourceMissing[0];
    } else if (
      runId &&
      currentSourceMissing.length === 1 &&
      currentSourceMissing[0].id === runId
    ) {
      legacy = currentSourceMissing[0];
    } else {
      throw new AppError(
  ...
  const filePathById = new Map(
    fileRows.map(file => [file.id, normalizePath(file.filePath)])
  );
  ...
  const snapshotSymbols = symbolRows.map(symbol => {
    ...
    const stableKey =
      metadata?.stableKey ??
      `${filePathById.get(symbol.fileId) ?? "unknown"}::${symbol.name}::${symbol.startLine}`;
  ...
  const snapshot: AnalysisRunSnapshotV1 = {
    ...
    dependencies: sortByJson(
      dependencyRows.map(dependency => ({
        from:
          symbolStableById.get(dependency.sourceSymbolId) ??
          `symbol:${dependency.sourceSymbolId}`,
        to: dependency.targetSymbolId
          ? symbolStableById.get(dependency.targetSymbolId)
          : undefined,
        fromName:
          symbolRows.find(symbol => symbol.id === dependency.sourceSymbolId)
            ?.name ?? `symbol:${dependency.sourceSymbolId}`,
        toName:
          dependency.targetExternalName ??
          (dependency.targetSymbolId
            ? symbolRows.find(symbol => symbol.id === dependency.targetSymbolId)
                ?.name
            : null) ??
          "unknown",
    ...
    fieldDependencies: sortByJson(
      fieldDependencyRows.map(dependency => {
  ...
  await db
    ...
    .set({
      ...
      sourceFingerprint:
        legacy.sourceFingerprint ?? calculateSourceFingerprint(fileRows),
...
export async function listAnalysisRuns(
  ...
  const rows = await db
    ...
    .from(analysisResults)
    .where(usableCondition)
    .orderBy(desc(analysisResults.runNumber), desc(analysisResults.id))
    .limit(pageSize)
    .offset(offset);
...
export async function setAnalysisBaseline(
  db: DbHandle,
  projectId: number,
  runId: number
) {
  ...
  if (existing) {
    await db
      .update(analysisBaselines)
      .set({ analysisResultId: runId, updatedAt: new Date() })
      .where(eq(analysisBaselines.projectId, projectId));
  } else {
    await db
      .insert(analysisBaselines)
      .values({ projectId, analysisResultId: runId });
  }
...
function bucket<T>(items: T[]) {
  ...
  return {
    items: filtered.slice(0, DIFF_LIMIT),
    total: filtered.length,
    displayed: Math.min(filtered.length, DIFF_LIMIT),
    truncated: filtered.length > DIFF_LIMIT,
  };
}
...
function diffEntries<T>(base: Map<string, T>, compare: Map<string, T>) {
  const keys = Array.from(new Set([...base.keys(), ...compare.keys()])).sort(
...
function buildDoctorFindingKey(
  item: AnalysisRunSnapshotV1["buildDoctor"]["findings"][number]
) {
...
function fieldDependencyKey(
  item: AnalysisRunSnapshotV1["fieldDependencies"][number]
) {
  const symbolIdentity = normalize(
    item.symbolStableKey ?? item.symbolName ?? "unknown"
  );
  return [
...
async function getComparableRunDetail(
  db: DbHandle,
  projectId: number,
  runId: number
) {
  const [run] = await db
    .select()
    .from(analysisResults)
    .where(eq(analysisResults.id, runId))
    .limit(1);
  if (!run) {
  ...
  if (run.projectId !== projectId) {
    throw new AppError(
      "PROJECT_NOT_FOUND",
      "Analysis runs from another project cannot be compared here."
    );
  }
  if (!isUsableStatus(run.status)) {
    throw new AppError(
...
export async function getAnalysisDiff(
  db: DbHandle,
  projectId: number,
  baseRunId: number,
  compareRunId: number
  ...
  if (baseRunId === compareRunId) {
    throw new AppError(
      "REPORT_NOT_READY",
      "An analysis run cannot be compared with itself."
    );
  }

  const [base, compare] = await Promise.all([
    getComparableRunDetail(db, projectId, baseRunId),
    getComparableRunDetail(db, projectId, compareRunId),
  ]);
  if (
    base.snapshot.schemaVersion !== ANALYSIS_SNAPSHOT_SCHEMA_VERSION ||
    compare.snapshot.schemaVersion !== ANALYSIS_SNAPSHOT_SCHEMA_VERSION
  ) {
    throw new AppError(
      "UNSUPPORTED_SNAPSHOT_VERSION",
      "Only immutable snapshot schema version 1 can be compared."
    );
  }

  const baseSnapshot = base.snapshot;
  const compareSnapshot = compare.snapshot;
  const fileDiff = diffEntries(
    keyed(baseSnapshot.sourceManifest, fileKey),
    keyed(compareSnapshot.sourceManifest, fileKey)
  );
  const symbolDiff = diffEntries(
    keyed(baseSnapshot.symbols, item => item.stableKey),
    keyed(compareSnapshot.symbols, item => item.stableKey)
  );
  const dependencyDiff = diffEntries(
    keyed(baseSnapshot.dependencies, dependencyKey),
    keyed(compareSnapshot.dependencies, dependencyKey)
  );
  const fieldDiff = diffEntries(
    keyed(baseSnapshot.fields, fieldKey),
    keyed(compareSnapshot.fields, fieldKey)
  );
  const fieldDependencyDiff = diffEntries(
    keyed(baseSnapshot.fieldDependencies, fieldDependencyKey),
    keyed(compareSnapshot.fieldDependencies, fieldDependencyKey)
  );
  const riskDiff = diffEntries(
    keyed(baseSnapshot.risks, riskKey),
    keyed(compareSnapshot.risks, riskKey)
  );
  const ruleDiff = diffEntries(
    keyed(baseSnapshot.rules, ruleKey),
    keyed(compareSnapshot.rules, ruleKey)
  );
  const eventDiff = diffEntries(
    keyed(baseSnapshot.delphiEventMap, eventKey),
    keyed(compareSnapshot.delphiEventMap, eventKey)
  );
  const bindingDiff = diffEntries(
    keyed(baseSnapshot.delphiDataBindings, bindingKey),
    keyed(compareSnapshot.delphiDataBindings, bindingKey)
  );
  const buildDiff = diffEntries(
    keyed(baseSnapshot.buildDoctor.findings, buildDoctorFindingKey),
    keyed(compareSnapshot.buildDoctor.findings, buildDoctorFindingKey)
  );
  const flowDiff = diffEntries(
    keyed(baseSnapshot.flowTraces, flowTraceKey),
    keyed(compareSnapshot.flowTraces, flowTraceKey)
  );
  const metricsDelta = Object.fromEntries(
    ANALYSIS_DIFF_METRIC_KEYS.map(key => [
      key,
      Number(compareSnapshot.metrics[key] ?? 0) -
        Number(baseSnapshot.metrics[key] ?? 0),
    ])
  );
  ...
  const result = {
    ...
    metricsDelta,
    files: {
      added: bucket(fileDiff.added),
      removed: bucket(fileDiff.removed),
      changed: bucket(fileDiff.changed),
    },
    symbols: {
      added: bucket(symbolDiff.added),
      removed: bucket(symbolDiff.removed),
    },
    dependencies: {
      added: bucket(dependencyDiff.added),
      removed: bucket(dependencyDiff.removed),
    },
    fields: {
      added: bucket(fieldDiff.added),
      removed: bucket(fieldDiff.removed),
      changed: bucket(fieldDiff.changed),
    },
    fieldDependencies: {
      introduced: bucket(fieldDependencyDiff.added),
      removed: bucket(fieldDependencyDiff.removed),
      changed: bucket(fieldDependencyDiff.changed),
    },
    risks: {
      introduced: bucket(riskDiff.added),
      resolved: bucket(riskDiff.removed),
      changed: bucket(riskDiff.changed),
    },
    rules: {
      introduced: bucket(ruleDiff.added),
      resolved: bucket(ruleDiff.removed),
      changed: bucket(ruleDiff.changed),
    },
    delphiEvents: {
      introduced: bucket(eventDiff.added),
      removed: bucket(eventDiff.removed),
      resolutionChanged: bucket(eventDiff.changed),
    },
    dataBindings: {
      introduced: bucket(bindingDiff.added),
      removed: bucket(bindingDiff.removed),
      changed: bucket(bindingDiff.changed),
    },
    buildDoctor: {
      introduced: bucket(buildDiff.added),
      resolved: bucket(buildDiff.removed),
      changed: bucket(buildDiff.changed),
      scoreDelta:
        compareSnapshot.buildDoctor.score - baseSnapshot.buildDoctor.score,
    },
    flowTraces: {
      introduced: bucket(flowDiff.added),
      removed: bucket(flowDiff.removed),
      changed: bucket(flowDiff.changed),
    },
    truncated: false,
  };
  result.truncated =
...
export async function getFlowTracesFromRun(
  db: DbHandle,
  projectId: number,
  runId?: number
) {
...
export async function getStrictSnapshotRunDetail(
  db: DbHandle,
  projectId: number,
  runId: number
) {
...
export function filterFlowTraces(
  traces: DelphiFlowTrace[],
  input: {
    search?: string;
    form?: string;
    component?: string;
  ...
  return traces.filter(trace => {
    if (search && !stableJson(trace).toLowerCase().includes(search))
      return false;
    if (input.form && normalize(trace.formName) !== normalize(input.form))
      return false;
    if (
      input.component &&
      normalize(trace.componentName) !== normalize(input.component)
    )
      return false;
    if (input.event && normalize(trace.eventName) !== normalize(input.event))
      return false;
    if (input.status && trace.status !== input.status) return false;
    if (input.confidence && trace.confidence !== input.confidence) return false;
    if (
      input.table &&
      !trace.affectedTables.some(
        table => normalize(table) === normalize(input.table)
      )
    )
      return false;
    if (
      input.operation &&
      !trace.affectedFields.some(field => field.operation === input.operation)
    )
      return false;
    return true;
  });

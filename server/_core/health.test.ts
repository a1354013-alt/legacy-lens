...
vi.mock("../services/projectWorkflow", () => ({
  getProjectJobWorkerHealthState: vi.fn(() => ({
    enabled: true,
    status: "up",
    reason: null,
  })),
...
describe("health endpoints", () => {
  ...
  it("/ready and /api/health fail when the database is unavailable", async () => {
    ...
    await withHealthServer(async baseUrl => {
      ...
      const readinessBody = (await readinessResponse.json()) as Record<
        string,
        unknown
      >;
      expect(readinessResponse.status).toBe(503);
      expect(readinessBody.status).toBe("not_ready");
      expect(readinessBody.reasonCodes).toEqual(["database_unavailable"]);
      expect(readinessBody).not.toHaveProperty("error");
      ...
      expect(healthBody).not.toHaveProperty("error");
    });
  });
});

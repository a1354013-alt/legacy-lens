...
ALTER TABLE `analysisResults` ADD `status` enum('pending','processing','completed','completed_with_warnings','partial','failed') DEFAULT 'pending' NOT NULL;--> statement-breakpoint
ALTER TABLE `analysisResults` ADD `summaryJson` json;--> statement-breakpoint
ALTER TABLE `analysisResults` ADD `warningsJson` json DEFAULT (JSON_ARRAY()) NOT NULL;--> statement-breakpoint

...
`pnpm audit --audit-level high` passes for the current v1.1.0 lockfile.

`pnpm audit --audit-level moderate` passes for the current v1.1.0 lockfile. The audit output reports 2 low findings below the release-required moderate threshold.
